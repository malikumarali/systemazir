...
export async function POST(req: NextRequest) {
  ...
  if (!isSupabaseConfigured) {
    const deleted = DemoStore.getOutbound().filter(e => ids.includes(e.id)).map(e => e.id)
    DemoStore.deleteOutboundBatch(ids)
    return ok({ deleted, count: deleted.length })
  }

  const admin = getSupabaseAdmin()
  if (!admin) return err('Server error', 500)
  const { error } = await admin.from('outbound_entries').delete().in('id', ids)
